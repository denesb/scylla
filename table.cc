...
table::make_sstable_reader(schema_ptr s,
    ...
    if (pr.is_singular() && pr.start()->value().has_key()) {
        ...
        return sstables->make_local_shard_sstable_reader(std::move(s), std::move(permit), pr, slice, pc, std::move(trace_state), fwd, fwd_mr);
    }
}
...
future<row_locker::lock_holder> table::do_push_view_replica_updates(const schema_ptr& s, mutation&& m, db::timeout_clock::time_point timeout, mutation_source&& source,
    ...
    if (cr_ranges.empty()) {
        ...
        return generate_and_propagate_view_updates(base, sem.make_tracking_only_permit(s.get(), "push-view-updates-1"), std::move(views), std::move(m), { }, std::move(tr_state), now).then([] {
