...
static future<bool> do_validate_compressed(input_stream<char>& stream, sstables::compression& c, bool checksum_all, uint32_t expected_digest) {
    bool valid = true;
    uint64_t offset = 0;
...
future<bool> sstable::validate_checksums(reader_permit permit, const io_priority_class& pc) {
    file_input_stream_options options;
    options.buffer_size = sstable_buffer_size;
    options.io_priority_class = pc;

    const auto digest = co_await read_digest(filename(component_type::Digest), options);

    auto data_file = make_tracked_file(_data_file, std::move(permit));
    auto data_stream = make_file_input_stream(std::move(data_file), options);
    ...
    try {
        if (_components->compression) {
            if (_version >= sstable_version_types::mc) {
                valid = co_await do_validate_compressed<crc32_utils>(data_stream, _components->compression, true, digest);
            } else {
                valid = co_await do_validate_compressed<adler32_utils>(data_stream, _components->compression, false, digest);
            }
        } else {
            auto checksum = co_await read_checksum(filename(component_type::CRC), options);
            if (_version >= sstable_version_types::mc) {
