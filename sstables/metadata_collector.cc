...
void metadata_collector::update_min_max_components(position_in_partition_view pos) {
    ...
    // We need special treatment for non-full clustering row keys of
    // compact-storage tables. We want to treat these like a range:
    // before_key(pos) and after_key(pos) for the purpose of calculating min and
    // max respectively.
