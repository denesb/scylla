...
future<> compaction_manager::perform_sstable_validation(column_family* cf) {
    return run_custom_job(cf, sstables::compaction_type::Validation, [this, &cf = *cf, sstables = get_candidates(*cf)] () mutable -> future<> {
        ...
        return do_with(std::move(sstables), pending_tasks(_stats, sstables.size()),
                [this, &cf] (std::vector<sstables::shared_sstable>& sstables, pending_tasks& pending) {
            return do_until([&sstables] { return sstables.empty(); }, [&] {
                auto sst = sstables.back();
                sstables.pop_back();

                return with_scheduling_group(_maintenance_sg.cpu, [&, sst] () {
                ...
                }).then_wrapped([&, sst] (future<sstables::compaction_info> f) {
                    try {
                        f.get();
