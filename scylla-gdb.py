...
class scylla_reads(gdb.Command):
    """List all active reads on the shard for each semaphore.

    Reads are discovered through the reader_permit they own. A read is
    considered to be the group readers that belong to the same permit.
    Semaphores, which have no associated reads are omitted from the printout.

    Example:
    (gdb) scylla reads
    Semaphore _read_concurrency_sem with: 1/100 count and 14334414/14302576 memory resources, queued: 0, inactive=1
       permits count       memory table/op name/state
    ...
    """
    def __init__(self):
        gdb.Command.__init__(self, 'scylla reads', gdb.COMMAND_USER, gdb.COMPLETE_COMMAND)

    def _dump_reads_from_semaphore(self, semaphore):
        try:
        ...
        gdb.write("{:>10} {:5} {:>12} {}\n".format('permits', 'count', 'memory', 'table/op name/state'))
    ...
    def invoke(self, args, from_tty):
        db = find_db()
        semaphores = [db["_read_concurrency_sem"], db["_streaming_concurrency_sem"], db["_compaction_concurrency_sem"], db["_system_read_concurrency_sem"]]
        for semaphore in semaphores:
            self._dump_reads_from_semaphore(semaphore)
