...
future<exceptions::coordinator_result<storage_proxy_coordinator_query_result>> query_nonmaterialized_view(
    ...
    auto base_table_slice_builder = partition_slice_builder(*base_query_schema, cmd->slice);

    base_table_slice_builder.with_no_static_columns();
    for (const auto& col_id : cmd->slice.static_columns) {
        base_table_slice_builder.with_static_column(view_schema->column_at(column_kind::static_column, col_id).name());
    }

    base_table_slice_builder.with_no_regular_columns();
    for (const auto& col_id : cmd->slice.regular_columns) {
        base_table_slice_builder.with_regular_column(view_schema->column_at(column_kind::regular_column, col_id).name());
    }

    auto base_table_slice = base_table_slice_builder.build();
