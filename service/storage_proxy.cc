...
const std::unordered_set<const locator::node*> to_nodes(const locator::topology& topo, const Range& eps) {
    return boost::copy_range<std::unordered_set<const locator::node*>>(eps | boost::adaptors::transformed([&] (gms::inet_address ep) {
        return topo.find_node(ep);
    }));
}
...
public:
    ...
    virtual bool store_hint(db::hints::manager& hm, const locator::node* node, tracing::trace_state_ptr tr_state) = 0;
...
public:
    ...
    virtual bool store_hint(db::hints::manager& hm, const locator::node* node, tracing::trace_state_ptr tr_state) override {
        auto m = _mutations[node->endpoint()];
        if (m) {
            return hm.store_hint(node, _schema, std::move(m), tr_state);
        } else {
            return false;
        }
    }
...
public:
    ...
    virtual bool store_hint(db::hints::manager& hm, const locator::node* node, tracing::trace_state_ptr tr_state) override {
            return hm.store_hint(node, _schema, _mutation, tr_state);
    }
    virtual future<> apply_locally(storage_proxy& sp, storage_proxy::clock_type::time_point timeout,
            tracing::trace_state_ptr tr_state, db::per_partition_rate_limit::info rate_limit_info) override {
        tracing::trace(tr_state, "Executing a mutation locally");
...
protected:
    ...
    template <typename Range>
    const std::unordered_set<const locator::node*> to_nodes(const Range& eps) const {
...
public:
    ...
    std::unordered_set<const locator::node*> get_target_nodes() const {
        return to_nodes(_targets);
    }
    const inet_address_vector_topology_change& get_dead_endpoints() const {
        return _dead_endpoints;
    }
    std::unordered_set<const locator::node*> get_dead_nodes() const {
        return to_nodes(_dead_endpoints);
    }
    bool store_hint(db::hints::manager& hm, const locator::node* node, tracing::trace_state_ptr tr_state) {
        return _mutation_holder->store_hint(hm, node, tr_state);
    }
...
size_t storage_proxy::hint_to_dead_endpoints(std::unique_ptr<mutation_holder>& mh, const Range& targets, db::write_type type, tracing::trace_state_ptr tr_state) noexcept
{
    if (hints_enabled(type)) {
        ...
        return boost::count_if(targets, [&mh, tr_state = std::move(tr_state), &hints_manager] (const locator::node* node) mutable -> bool {
            return mh->store_hint(hints_manager, node, tr_state);
        });
    } else {
        return 0;
    }
