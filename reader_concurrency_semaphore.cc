...
public:
    ...
    void mark_blocked() noexcept {
        assert(_state == reader_permit::state::active_used || _state == reader_permit::state::active_blocked);
        ++_blocked_branches;
        if (_blocked_branches == 1) {
...
bool reader_concurrency_semaphore::all_used_permits_are_stalled() const {
    return _permit_list->used_permits == _permit_list->blocked_permits || _permit_list->total_permits == 1;
}
