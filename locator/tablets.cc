...
void tablet_map::drop_tablets(const std::vector<tablet_id>& dropped_tablets) {
    const auto new_tablet_count = _tablets.size() - dropped_tablets.size();
    const auto new_log2_tablets = log2ceil(new_tablet_count);

    if (new_tablet_count != 1ul << new_log2_tablets) {
        throw std::runtime_error(format("drop_tablets(): tablet count after dropping tablets is not a power of 2: {}", new_tablet_count));
    }

    tablet_container new_tablets;
    new_tablets.reserve(new_tablet_count);

    for (size_t tid = 0; tid < _tablets.size(); ++tid) {
        if (std::ranges::find(dropped_tablets, tablet_id(tid)) == dropped_tablets.end()) {
            new_tablets.push_back(std::move(_tablets[tid]));
        } else {
            _transitions.erase(tablet_id(tid));
        }
    }

    std::swap(_tablets, new_tablets);
    _log2_tablets = new_log2_tablets;
}
