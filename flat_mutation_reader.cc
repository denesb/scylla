...
make_flat_mutation_reader_from_fragments(schema_ptr schema, reader_permit permit, std::deque<mutation_fragment> fragments,
    ...
    auto rd = make_flat_mutation_reader_from_fragments(std::move(schema), std::move(permit), std::move(filtered), pr);
    if (reversed) {
        rd = make_reversing_reader(std::move(rd), query::max_result_size(query::result_memory_limiter::unlimited_result_size));
    }
    return rd;
}
