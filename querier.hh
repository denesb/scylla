...
protected:
    ...
    static std::unique_ptr<const query::partition_slice> make_slice(query::partition_slice slice) {
        auto slice_ptr = std::make_unique<query::partition_slice>(std::move(slice));
        if (slice_ptr->options.contains(query::partition_slice::option::reversed)) {
            // We have to convert the slice back to the legacy reversed form,
            // until the lower layers support reversing natively
            query::native_reverse_slice_to_legacy_reverse_slice(*slice_ptr);
        }
        return slice_ptr;
    }
...
public:
    ...
    querier_base(schema_ptr schema, reader_permit permit, dht::partition_range range,
        ...
        , _slice(make_slice(std::move(slice)))
