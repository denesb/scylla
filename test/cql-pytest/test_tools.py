...
# The following tests test sstable validation.
#
# Even though these tests are in test_tools.py, their main focus is not on the
# `scylla sstable` tool itself, but the underlying validation facilities.
...
def test_scylla_sstable_validate_mismathing_position(cql, scylla_path, temp_workdir, schema1_file, sstable_cache):
    validate_mixed_sstable_pair(
...
def test_scylla_sstable_validate_large_rows_end_of_pi_not_end_of_rows(cql, scylla_path, temp_workdir, schema1_file, sstable_cache, large_rows):
    row_0x = { "type": "clustering-row", "key": { "raw": "000400000000" }, "columns": { "v": { "is_live": True, "type": "regular", "timestamp": 1680263186359882, "value": "v" } } }
    # mismatching index/data promoted index has more clustering elements than data file
    validate_mixed_sstable_pair(
