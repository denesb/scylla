...
private:
    ...
    void run_in_thread(std::function<future<>(cql_test_env&)> func, cql_test_config cfg_in, std::optional<cql_test_init_configurables> init_configurables) {
            ...
            _cm.invoke_on_all([&](compaction_manager& cm) {
                ...
                if (cl && scl) {
                    cm.get_tombstone_gc_state().set_gc_time_min_source([cl, scl](const table_id& id, const db::rp_set* exclude) {
                        return std::min(cl->min_gc_time(id, exclude), scl->min_gc_time(id, exclude));
                    });
                } else if (cl) {
                    cm.get_tombstone_gc_state().set_gc_time_min_source([cl](const table_id& id, const db::rp_set* exclude) {
                        return cl->min_gc_time(id, exclude);
                    });
                } else if (scl) {
                    cm.get_tombstone_gc_state().set_gc_time_min_source([scl](const table_id& id, const db::rp_set* exclude) {
