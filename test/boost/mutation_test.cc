...
SEASTAR_THREAD_TEST_CASE(test_compactor_range_tombstone_spanning_many_pages) {
    ...
    auto make_frags = [&] {
        ...
        auto marker_ts = ss.new_timestamp();
        auto tomb_ts = ss.new_timestamp();
        auto row_ts = ss.new_timestamp();

        const auto& v_def = *s->get_column_definition(to_bytes("v"));

        frags.emplace_back(*s, permit, range_tombstone_change(position_in_partition::before_key(ss.make_ckey(1)), tombstone{tomb_ts, {}}));

        for (uint32_t ck = 0; ck < 10; ++ck) {
            auto row = clustering_row(ss.make_ckey(format("{}", ck)));
