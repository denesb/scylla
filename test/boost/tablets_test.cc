...
SEASTAR_TEST_CASE(test_tablet_metadata_update) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        // Stream a tablet of table2
        {
            testlog.trace("stream a tablet of table2");
            ...
            const auto& tmap = tm.get_tablet_map(table2);
            auto tb = tmap.first_tablet();
            testlog.trace("tmap={}", tmap);
            builder.set_new_replicas(tmap.get_last_token(tb),
                tablet_replica_set {
                    tablet_replica {h2, 7},
                }
            );
            builder.set_stage(tmap.get_last_token(tb), tablet_transition_stage::streaming);
