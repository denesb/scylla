...
SEASTAR_THREAD_TEST_CASE(test_reader_concurrency_semaphore_forward_progress) {
    ...
    public:
        reader(schema_ptr s, reader_concurrency_semaphore& semaphore, unsigned reader_id, bool memory_only, bool evictable)
            : _schema(std::move(s))
    ...
    for (auto i = 0; i <  num_readers; ++i) {
        ...
        readers.emplace_back(std::make_unique<reader>(s.schema(), semaphore, i, memory_only, evictable));
    }
...
SEASTAR_THREAD_TEST_CASE(test_reader_concurrency_semaphore_used_blocked) {
    ...
    BOOST_REQUIRE_EQUAL(semaphore.get_permit_stats().total_permits, 0);
    ...
    // The order of the first two is important.
    used.emplace_back(permit);

    BOOST_REQUIRE_EQUAL(semaphore.get_permit_stats().total_permits, 1);
