...
static reconcilable_result mutation_query(schema_ptr s, reader_permit permit, const mutation_source& source, const dht::partition_range& range,
    ...
    auto result_schema = slice.options.contains(query::partition_slice::option::reversed) ? s->make_reversed() : s;
    auto rrb = reconcilable_result_builder(*result_schema, slice, make_accounter());
