...
SEASTAR_TEST_CASE(test_missing_partition_end_fragment) {
    return test_setup::do_with_tmp_directory([] (test_env& env, sstring tmpdir_path) {
        ...
        for (const auto version : writable_sstable_versions) {
            ...
            frags.emplace_back(*s, env.make_reader_permit(), partition_start(pkeys[0], tombstone()));
            frags.emplace_back(*s, env.make_reader_permit(), clustering_row(ss.make_ckey(0)));
            // partition_end is missing
            frags.emplace_back(*s, env.make_reader_permit(), partition_start(pkeys[1], tombstone()));
            frags.emplace_back(*s, env.make_reader_permit(), clustering_row(ss.make_ckey(0)));
            frags.emplace_back(*s, env.make_reader_permit(), partition_end());
