...
SEASTAR_THREAD_TEST_CASE(sstable_scrub_reader_and_validation_test) {
    auto schema = schema_builder("ks", get_name())
            .with_column("pk", utf8_type, column_kind::partition_key)
            .with_column("ck", int32_type, column_kind::clustering_key)
            .with_column("s", int32_type, column_kind::static_column)
...

    {
        ...
        for (const auto& mf : corrupt_fragments) {
            buf.emplace_back(*schema, permit, mf);
        }
        const auto corrupt_valid = validate_compaction_validate_reader(make_flat_mutation_reader_from_fragments(schema, permit, std::move(buf)), *info).get();
        BOOST_REQUIRE(!corrupt_valid);

        for (const auto& mf : scrubbed_fragments) {
            buf.emplace_back(*schema, permit, mf);
        }
        const auto scrubbed_valid = validate_compaction_validate_reader(make_flat_mutation_reader_from_fragments(schema, permit, std::move(buf)), *info).get();
        BOOST_REQUIRE(scrubbed_valid);
    }
