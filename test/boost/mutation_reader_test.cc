...
SEASTAR_THREAD_TEST_CASE(test_evictable_reader_drop_flags) {
    ...
    struct buffer {
        ...
        size_t add_mutation_fragment(mutation_fragment&& mf) {
            muts.back().apply(mf);
        ...
        size_t add_clustering_row(int i) {
            return add_mutation_fragment(mutation_fragment(*s.schema(), permit, s.make_row(s.make_ckey(i), "v")));
        }
    ...
    testlog.info("Next partition, with no static row");
    {
        ...
        second_buffer.add_clustering_rows(0, second_buffer_ck);
        second_buffer.add_partition_end();
