...
SEASTAR_THREAD_TEST_CASE(test_reader_concurrency_semaphore_admission) {
    ...
    auto impl_ptr = std::make_unique<reader>(s.schema(), semaphore.obtain_permit(s.schema().get(), get_name(), 1024, db::no_timeout).get0());
    auto& impl = *impl_ptr;
...
SEASTAR_TEST_CASE(reader_concurrency_semaphore_max_queue_length) {
    return async([&] () {
...

        {
            ...
            reader_permit_opt permit2 = permit2_fut.get();
            permit2 = {};

            reader_permit_opt permit3 = permit3_fut.get();
            permit3 = {};
        }
