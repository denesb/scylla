...
SEASTAR_THREAD_TEST_CASE(test_evictable_reader_non_monotonic_positions) {
    ...
    auto reader = make_auto_paused_evictable_reader_v2(std::move(ms), schema, permit, dht::partition_range::make_open_ended_both_sides(),
            schema->full_slice(), seastar::default_priority_class(), nullptr, mutation_reader::forwarding::no);
    auto close_reader = deferred_close(reader);
...
SEASTAR_THREAD_TEST_CASE(test_evictable_reader_clear_tombstone_in_discontinued_partition) {
    ...
    auto check = [&] (const std::deque<mutation_fragment_v2>& second_buffer, const char* scenario) {
        ...
        auto ms = mutation_source([&first_buffer, &second_buffer, first = true] (
                schema_ptr schema,
                reader_permit permit,
                const dht::partition_range& range,
                const query::partition_slice& slice,
