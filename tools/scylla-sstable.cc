...
void validate_checksums_operation(schema_ptr schema, reader_permit permit, const std::vector<sstables::shared_sstable>& sstables, const bpo::variables_map&) {
    for (auto& sst : sstables) {
        const auto valid = sst->validate_checksums(permit, default_priority_class()).get();
...
sstables. The digest and the per-chunk checksum of uncompressed sstables is not
