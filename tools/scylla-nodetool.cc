...
void tablehistograms_operation(scylla_rest_client& client, const bpo::variables_map& vm) {
    ...
    const auto read_latency_hg = client.get(format("/column_family/metrics/read_latency/moving_average_histogram/{}:{}", keyspace, table));
    const auto write_latency_hg = client.get(format("/column_family/metrics/write_latency/moving_average_histogram/{}:{}", keyspace, table));
