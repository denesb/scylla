...
void configure_tool_mode(app_template::seastar_options& opts, const sstring& logger_name) {
    ...
    opts.reactor_opts.overprovisioned.set()
    opts.reactor_opts.thread_affinity.set_value(false);
    opts.reactor_opts.idle_poll_time_us.set_value(0);
    opts.reactor_opts.poll_aio.set_value(false);
    opts.reactor_opts.relaxed_dma.set_value(true);
    ...
    if (!logger_name.empty()) {
        opts.logger_log_level.set_value({});
        opts.logger_log_level.get_value()[logger_name] = log_level::info;
    }
}
