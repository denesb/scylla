...
memtable::make_flat_reader(schema_ptr s, reader_permit permit, const dht::partition_range& range, const query::partition_slice& query_slice,
    ...
    // When the memtable is flushed while a read is ongoing, an sstable reader is
    // created to replace the memtable reader mid-air. This sstable reader will
    // get the memtable's slice. We don't want this sstable reader to read in
    // reverse as we are reversing the stream on top of the memtable reader.
    // Unreverse the slice here so when it is passed to the sstable reader it doesn't try to read in reverse.
