...
future<page_consume_result<ResultBuilder>> read_page(
    ...
    auto compaction_state = make_lw_shared<compact_for_query_state>(*s, cmd.timestamp, cmd.slice, cmd.get_row_limit(),
            cmd.partition_limit, table.table_tombstone(), mutation_fragment_stream_validation_level::token, tombstone_gc_enabled);
    ...
    auto f = co_await coroutine::as_future(consume_page(reader, compaction_state, cmd.slice, result_builder_factory(), cmd.get_row_limit(),
                cmd.partition_limit, cmd.timestamp, table.table_tombstone(), tombstone_gc_enabled));
    if (!f.failed()) {
...
future<foreign_ptr<lw_shared_ptr<typename ResultBuilder::result_type>>> do_query_vnodes(
    ...
    auto f = co_await coroutine::as_future(ctx->lookup_readers(timeout).then([&, result_builder_factory = std::move(result_builder_factory)] () mutable {
        return read_page<ResultBuilder>(ctx, s, table, cmd, ranges, trace_state, tombstone_gc_enabled, std::move(result_builder_factory));
