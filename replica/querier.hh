...
auto consume_page(mutation_reader& reader,
    ...
    return reader.peek().then([=, &reader, consumer = std::move(consumer)] (
        ...
        compaction_state->start_new_page(row_limit, partition_limit, query_time, table_tombstone, tombstone_gc_enabled, next_fragment_region, consumer);
...
public:
    ...
    auto consume_page(Consumer&& consumer,
        ...
        return ::replica::consume_page(std::get<mutation_reader>(_reader), _compaction_state, *_slice, std::move(consumer), row_limit,
                partition_limit, query_time, table_tombstone, tombstone_gc_enabled).then_wrapped([this, trace_ptr = std::move(trace_ptr)] (auto&& fut) {
