...
table::query(schema_ptr query_schema,
    ...
    while (!qs.done()) {
        ...
        if (!querier_opt) {
            ...
            querier_opt = querier(as_mutation_source(), query_schema, permit, range, qs.cmd.slice, _table_tombstone, trace_state, conf);
        }
        auto& q = *querier_opt;
...
table::mutation_query(schema_ptr query_schema,
    ...
    if (!querier_opt) {
        querier_base::querier_config conf(_config.tombstone_warn_threshold);
        querier_opt = querier(as_mutation_source(), query_schema, permit, range, cmd.slice, _table_tombstone, trace_state, conf);
    }
    auto& q = *querier_opt;
  ...
  try {
    ...
    auto r = co_await q.consume_page(std::move(rrb), cmd.get_row_limit(), cmd.partition_limit, cmd.timestamp, _table_tombstone, trace_state, tombstone_gc_enabled);
