...
class shard_reader : public enable_lw_shared_from_this<shard_reader>, public flat_mutation_reader::impl {
    ...
    private:
        ...
        future<> ensure_safe_progress(flat_mutation_reader& reader, circular_buffer<mutation_fragment>& buffer, db::timeout_clock::time_point timeout);
...
future<> shard_reader::remote_reader::ensure_safe_progress(flat_mutation_reader& reader,
    ...
    auto stop = [this, &reader, &buffer] {
        ...
        // * Make sure the buffer contains *at least two* distinct positions.
        //   Otherwise we run the risk of just reading the same
        //   range-tombstones re-emitted on each reader recreate, if the reader
        //   is evicted after each `fill_buffer()` call.
        return _tri_cmp(buffer.back().position(), *_last_position_in_partition) > 0 &&
                _tri_cmp(next_pos, position_in_partition_view::after_key(buffer.back().key())) > 0 &&
                _tri_cmp(buffer.front().position(), buffer.back().position()) < 0;
    };
