...
future<db::all_batches_replayed> db::batchlog_manager::replay_all_failed_batches(post_replay_cleanup cleanup) {
    ...
    std::unordered_map<int32_t, db_clock::time_point> min_written_at_per_shard;

    auto batch = [this, cleanup, limiter, schema, &min_written_at_per_shard, &all_replayed] (const cql3::untyped_result_set::row& row) -> future<stop_iteration> {
        ...
        try {
            ...
            if (now < written_at + timeout) {
                ...
                auto& shard_written_at = min_written_at_per_shard.at(row.get_as<int32_t>("shard"));
                shard_written_at = std::min(shard_written_at, written_at);
    ...
    co_await with_gate(_gate, [this, cleanup, &all_replayed, batch = std::move(batch), &min_written_at_per_shard] () mutable -> future<> {
            ...
            for (int32_t i = 0; i < 16; ++i) {
                ...
                co_await _qp.query_internal(
                 ...
                 co_await _qp.query_internal(
                        format("SELECT * FROM {}.{} WHERE version = ? AND stage = ? AND shard = ? BYPASS CACHE", system_keyspace::NAME, system_keyspace::BATCHLOG_V2),
                        db::consistency_level::ONE,
                        {data_value(netw::messaging_service::current_version), data_value(int32_t(batchlog_stage::initial)), data_value(batchlog_shard)},
                        page_size,
                        batch);

                if (cleanup == post_replay_cleanup::yes) {
                    auto [key, ckey] = get_batchlog_key(*schema, netw::messaging_service::current_version, batchlog_stage::initial, batchlog_shard, min_written_at_per_shard.at(batchlog_shard), {});
