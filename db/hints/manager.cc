...
sync_point::shard_rps manager::calculate_current_sync_point(std::unordered_set<const locator::node*>& target_hosts) const {
...
manager::end_point_hints_manager::end_point_hints_manager(const key_type& key, manager& shard_manager)
    ...
    , _hints_dir(_shard_manager.hints_dir() / format("{}", _key->endpoint()).c_str())
...
manager::end_point_hints_manager& manager::get_ep_manager(const locator::node* node) {
...
inline bool manager::have_ep_manager(const locator::node* node) const noexcept {
    return find_ep_manager(node) != ep_managers_end();
}

bool manager::store_hint(const locator::node* node, schema_ptr s, lw_shared_ptr<const frozen_mutation> fm, tracing::trace_state_ptr tr_state) noexcept {
...
bool manager::too_many_in_flight_hints_for(const locator::node* node) const noexcept {
    ...
    return _stats.size_of_hints_in_progress > max_size_of_hints_in_progress && node->is_this_node() && hints_in_progress_for(node) > 0 && local_gossiper().get_endpoint_downtime(node->endpoint()) <= _max_hint_window_us;
}

bool manager::can_hint_for(const locator::node* node) const noexcept {
...
bool manager::check_dc_for(const locator::node* node) const noexcept {
    try {
...
void manager::drain_for(const locator::node* node) {
