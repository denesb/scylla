...
using node_to_hint_store_factory_type = utils::loading_shared_values<const locator::node*, db::commitlog>;
...
public:
    ...
    future<> stop();
    bool store_hint(const locator::node* node, schema_ptr s, lw_shared_ptr<const frozen_mutation> fm, tracing::trace_state_ptr tr_state) noexcept;
    ...
    bool can_hint_for(const locator::node* node) const noexcept;
    ...
    bool too_many_in_flight_hints_for(const locator::node* node) const noexcept;
    ...
    bool check_dc_for(const locator::node* node) const noexcept;
    ...
    uint64_t hints_in_progress_for(const locator::node* node) const noexcept {
    ...
    sync_point::shard_rps calculate_current_sync_point(std::unordered_set<const locator::node*>& target_hosts) const;
...
private:
    ...
    bool have_ep_manager(const locator::node* node) const noexcept;
...
public:
    ...
    void drain_for(const locator::node* node);
