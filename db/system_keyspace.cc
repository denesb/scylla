...
class db_config_table final : public streaming_virtual_table {
    ...
    static schema_ptr build_schema(schema_registry& registry) {
        ...
        auto v = system_keyspace::generate_schema_version(id);
      return registry.get_or_load(v, [&registry, id] (table_schema_version v) {
        return schema_builder(registry, system_keyspace::NAME, "config", std::make_optional(id))
            .with_column("name", utf8_type, column_kind::partition_key)
...
public:
    explicit db_config_table(database& db, db::config& cfg)
            : streaming_virtual_table(build_schema(db.get_schema_registry()))
            , _cfg(cfg)
    {
        _shard_aware = true;
...
void register_virtual_tables(distributed<database>& dist_db, distributed<service::storage_service>& dist_ss, db::config& cfg) {
    ...
    add_table(std::make_unique<db_config_table>(db, cfg));
}
