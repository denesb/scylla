...
/// with_ready_permit().
class reader_concurrency_semaphore : public execution_stage {
public:
...
private:
    ...
    circular_buffer<entry> _ready_list;
...
public:
    ...
    /// it was created  with \ref obtain_permit(), then func is enqueued on
    /// the semaphore's execution stage.
    ///
    /// The permit is associated with a schema, which is the schema of the table
    /// the read is executed against, and the operation name, which should be a
    /// name such that we can identify the operation which created this permit.
